...
New Stuff:
Unlike the previous iteration, this program will actually use a GUI (tkinter), save features (csv), and classes. 
This could also be seen as a rewrite, since I'm switching everything over from console-based input to buttons
input windows.
All of the dictionary stuff that stores the product info through indices is being removed and replaced with classes,
since it caused too many issues.
Please note that I have used tkinter before, but not on something of this scale. Trust me, it might not go well.
"""
...
#Notes:
"""
These are the same notes from the last final, although anything different will be added onto at the end.
...
RANDOM NOTES FROM DEVELOPER (part 2: Regret and Revelations):
"""
